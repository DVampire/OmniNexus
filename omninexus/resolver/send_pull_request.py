...
from omninexus.core.config import LLMConfig
from omninexus.core.logger import omninexus_logger as logger
from omninexus.resolver.github_issue import GithubIssue
from omninexus.resolver.io_utils import (
    load_all_resolver_outputs,
    load_single_resolver_output,
)
from omninexus.resolver.patching import apply_diff, parse_patch
from omninexus.resolver.resolver_output import ResolverOutput
...
def make_commit(repo_dir: str, issue: GithubIssue, issue_type: str) -> None:
    ...
    if not result.stdout.strip():
        ...
        subprocess.run(
            f'git -C {repo_dir} config user.name "omninexus" && '
            f'git -C {repo_dir} config user.email "omninexus@all-hands.dev" && '
            f'git -C {repo_dir} config alias.git "git --no-pager"',
            shell=True,
            check=True,
        )
        print('Git user configured as omninexus')
    ...
    if not status_result.stdout.strip():
        ...
        raise RuntimeError('ERROR: omninexus failed to make code changes.')
...
def send_pull_request(
    ...
    base_branch_name = f'omninexus-fix-issue-{github_issue.number}'
    ...
    if additional_message:
        pr_body += f'\n\n{additional_message}'
    pr_body += '\n\nAutomatic fix generated by [omninexus](https://github.com/All-Hands-AI/omninexus/) 🙌'
...
def reply_to_comment(github_token: str, comment_id: str, reply: str):
    ...
    comment_reply = f'omninexus fix success summary\n\n\n{reply}'
...
def update_existing_pull_request(
    ...
    if not comment_message and additional_message:
        try:
            ...
            if explanations:
                comment_message = (
                    'omninexus made the following changes to resolve the issues:\n\n'
                )
            ...
            comment_message = 'New omninexus update'
