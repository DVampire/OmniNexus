...
The omninexus Runtime folder contains the core components responsible for executing actions and managing the runtime environment for the omninexus project. This README provides an overview of the main components and their interactions.
...
The `action_executor_server.py` file contains the `ActionExecutor` class, which is responsible for executing actions received from the omninexus backend and producing observations. This client runs inside a Docker sandbox.
...
6. **Sandbox Environment**:
   ...
   - Actions received from the omninexus backend are executed in this sandboxed environment.
...
This is the default runtime used within omninexus.
...
At the time of this writing, this is mostly used in parallel evaluation, such as this example for [SWE-Bench](https://github.com/All-Hands-AI/omninexus/tree/main/evaluation/swe_bench#run-inference-on-remoteruntime-experimental).

## Related Components

- The runtime interacts closely with the event system defined in the `omninexus.events` module.
- It relies on configuration classes from `omninexus.core.config`.
- Logging is handled through `omninexus.core.logger`.

This section provides an overview of the omninexus Runtime folder. For more detailed information on specific components or usage, please refer to the individual files and their docstrings.
